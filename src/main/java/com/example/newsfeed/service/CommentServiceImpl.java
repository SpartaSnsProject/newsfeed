...
public class CommentServiceImpl implements CommentService{
    ...
    public ResponseComment addComment(RequestComment requestComment, String username) {
        User user = userRepository.findByEmail(username).orElseThrow(() -> new RuntimeException("유저 못찾음"));
        ...
        if (requestComment.getCommentId() == null && requestComment.getPostId() == null) {
            ...
            Post post = postRepository.findById(requestComment.getPostId()).orElseThrow(() -> new RuntimeException("게시물 못찾음"));
            Comment save = commentRepository.save(new Comment(displayName, post, requestComment.getContents()));
